...
export function resolveDocumentImports<T>(
  ...
  const basePath = baseOutputDir.startsWith('./') ? baseOutputDir.split('./')[1] : baseOutputDir;

  documents.forEach(documentFile => {
    const moduleName = documentFile.location.split(basePath)[1].split('/')[0];
    const directoryGroup = directoryGroups.find(dirGroup => dirGroup.moduleName === moduleName);

    if (directoryGroup) {
      const directyGroupIndex = directoryGroups.indexOf(directoryGroup);
      directoryGroups.splice(directyGroupIndex, 1, {
        moduleName,
        documents: [...directoryGroup.documents, documentFile],
      });
    } else {
  ...
  return directoryGroups.map(group => {
    try {
      const generatedFilePath = `${basePath}${group.moduleName}/types.generated.ts`;
      const importStatements: string[] = [];
      const firstDocument = group.documents[0];
      const { externalFragments, fragmentImports } = resolveFragments(generatedFilePath, firstDocument.document);
