...
import { appendExtensionToFilePath } from './utils';
...
export type DirectoryGroupedConfig = {
  /**
...
export const preset: Types.OutputPreset<DirectoryGroupedConfig> = {
  buildGeneratesSection: options => {
    ...
    if (!baseTypesPath) {
      throw new Error(
        `Preset "directory-grouped" requires you to specify "baseTypesPath" configuration and point it to your base types file (generated by "typescript" plugin)!`
      );
    }
    ...
    const sources = resolveDocumentImports(options, schemaObject, {
      ...
      generateFilePath(location: string) {
        return appendExtensionToFilePath(location, extension);
      },
      schemaTypesSource: {
